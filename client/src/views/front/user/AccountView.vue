...
const fetchNewsletterSubscription = async () => {
  ...
  newsletterSubscription.value = newsletterAlert
    ? newsletterAlert.isActive
    : false;
};
  ...
  <div class="container mx-auto p-4">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div class="bg-white p-6 rounded shadow-lg relative">
        ...
        <div class="mt-4">
          <button
            ...
            class="underline mr-16 text-lg"
          >
            Modifier mon mot de passe
          </button>
          <RouterLink to="/account/delete" class="underline text-lg"
      ...
      <div class="bg-white p-6 rounded shadow-lg relative">
        ...
        <p class="text-lg" v-if="!authStore.user?.address">
          Vous n'avez renseigné aucune adresse.
        </p>
        <div v-else>
          <p class="text-lg">{{ authStore.user?.address }}</p>
          <p class="text-lg">
            {{ authStore.user?.postalCode }} {{ authStore.user?.city }}
          </p>
