...
import { EditUserSchema } from "@/z-schemas/UserSchema";
...
const { values, errors, isSubmitting, httpError, handleSubmit, setValues } =
  useForm({
    ...
    onSubmit: async (values) => {
      await usersStore.updateUser(userId, values);
      if (!usersStore.error) {
...
const fetchUser = async () => {
  ...
  setValues(flattenValues(userInfo.value));
};
  ...
  <div class="flex justify-between w-full">
    <span class="flex flex-col">
      ...
      <span class="text-md text-text-200"
        >Modifiez les détails de l'utilisateur.</span
      >
    </span>
    <Button
